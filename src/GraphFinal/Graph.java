...
//        System.out.println("dist test : " + getDistanceOrTimeBetweenTwoNodes(path.get(0), path.get(1), type));
    ...
    public ArrayList<Node> printShortByFiability(Node source, Node end){
        ...
        for(int i = 0; i < pathFiab.size()-1; i++){
            //System.out.println( pathFiab.get(i)+ " - "+ pathFiab.get(i+1) + "  fiab :" + getFiabiliteBetweenTwoNodes(pathFiab.get(i), pathFiab.get(i+1)));
            sum *= getFiabiliteBetweenTwoNodes(pathFiab.get(i), pathFiab.get(i+1));
        }
        //System.out.println("the sum in dikjstar is : " + sum );
        System.out.println(reversedPathFiab);
        
        //System.out.println("s1 - S3 " +  getFiabiliteBetweenTwoNodes(getNodeByString("S" + 1), getNodeByString("S" +2)));
                        ...
                        switch(type){
                            case 0:
                                if(Double.valueOf(val) > 10){
                                    ...
                                    n = new Edge( source,Double.valueOf(val),ed.getDist(), ed.getTemp());
                                    getAdjacentEdgeList(end).removeIf(pr2);
                            ...
                            case 1:
                                ...
                                n = new Edge( source, ed.getFiab(), Integer.parseInt(val), ed.getTemp());
                                getAdjacentEdgeList(end).removeIf(pr2);
                                getAdjacentEdgeList(end).add(n);
                            ...
                            case 2:
                                ...
                                n = new Edge( source, ed.getFiab(), ed.getDist(), Integer.parseInt(val));
                                getAdjacentEdgeList(end).removeIf(pr2);
                                getAdjacentEdgeList(end).add(n);
                                return true;
                        }
